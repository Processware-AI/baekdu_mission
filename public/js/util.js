...
/**
 * 가로로 넘겨 보는 칸막이(.filters) 의 위치 지키기.
 *
 * 칸을 누르면 화면을 통째로 다시 그리는데, 그때 넘겨둔 위치가 처음으로 돌아간다.
 * 오른쪽 끝에 있는 칸(운영진의 '사용현황' 같은)을 누르면 방금 누른 칸이
 * 화면 밖으로 사라져, 매번 다시 넘겨서 찾아야 했다.
 *
 * 화면 이름별로 넘겨둔 위치를 기억했다가 다시 그린 뒤 되돌려 놓고,
 * 그래도 고른 칸이 안 보이면 보이는 데까지만 밀어준다.
 */
const stripPos = new Map();
...
/** 고른 칸이 화면 밖이면 딱 보일 만큼만 밀어준다 (세로 위치는 건드리지 않는다) */
function showActiveChip(strip) {
